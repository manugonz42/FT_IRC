...
bool Server::processCommand(Client *client, const ParsedCommand &cmd)
{
	...
	if (cmd.command == "CAP") {
		...
		if (cmd.params.size() >= 2) {
			...
			{
				sendMessage(client->getFd(), "464 * :Password incorrect", PREFIX);
	...
	// }
		...
		if (cmd.params.size() >= 5 && client->isAuthenticated()) {
			...
			sendMessage(client->getFd(), "001 " + nick + " :Welcome to the IRC Network " + nick + "!", PREFIX);
			sendMessage(client->getFd(), "002 " + nick + " :Your host is server, running version 1.0", PREFIX);
			sendMessage(client->getFd(), "004 " + nick + " server 1.0", PREFIX);
			
			std::cout << "Client " << client->getFd() << " fully registered as " << nick << std::endl;
		} 
	}
	else if (cmd.command == "PING")
		sendMessage(client->getFd(), "PONG " + cmd.params[1], PREFIX);
